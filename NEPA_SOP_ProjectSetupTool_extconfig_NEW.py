...
The config file is a csv with the following fields
    ...
    added for summary tables:
    summary_index  - column to group by on the pivot table index
    summary_values - values to aggregate (sum)
    summary_column - column to group by on the pivot table column
...
***TODO
update the summary table functions when library is available in Citrix
for pandas to write to excel
'''
...
def summary_table_xls(gdb_path, feature_dataset, featureClass, index, values, column, outxls):
    """
    ...
    tbl_out = os.path.join(project_folder, "GIS", "AnyalysisProduct", "Archive", "{}.xls".format(featureClass))
...
def process_summary_tables_xls(config_file, out_xls):
    """
        ...
        for row in reader:
            try:
                if row['summary_values']:
                    ...
                    fc = os.path.join(fullpath, feature_dataset, feature_class)                    
...
# Create the project geodatabase
arcpy.CreateFileGDB_management(os.path.join(folder_loc, "GIS", "Data"), gdb_name)
arcpy.AddMessage("File geodatabase created successfully")

#Create feature datasets
fdList = ['ALT_DEVELOPMENT', 'DECISION', 'FireAndFuels',
          'Fisheries', 'Geophysical', 'Hydrology',
          'Surveys', 'Project', 'Range', 'Recreation',
          'Transportation', 'Undeveloped_Char', 'Vegetation',
          'Wildlife', 'Land', 'Activities', 'ManagementDirection',
          'CartographicReference', 'Reference']
for fd in fdList:
...
##----------------    create summary tables -------------------------
## added to the config file - summary_index, summary_values, summary_column
## waiting on a library that pandas needs in order to export to excel sheets
## update the function when it's available in Citrix
#
arcpy.AddMessage("Creating summaries")

## read the main config file and create summary tables
#process_summary_tables(config_file_extra1, fullpath, folder_loc)
#
#
## read the optional config files and create summary tables
#if config_file_extra1:
#    process_summary_tables(config_file_extra2, fullpath, folder_loc)



arcpy.AddMessage("Writing summary tables to excel")
# for excel writing, need to write all the sheets at once using ExcelWriter
excel_summary_tbl = os.path.join(folder_loc, "GIS", "AnalysisProduct",
                                 "{}_summary".format(projectName))
full_tblPath= os.path.join(folder_loc, "GIS", "AnalysisProduct")


process_summary_tables_xls(config_file_extra1, excel_summary_tbl) 

if config_file_extra2:
    process_summary_tables_xls(config_file_extra2, excel_summary_tbl)





# summary tables outside of the config file
try:
    summary_table_xls(fullpath, "Transportation", "INFRA_Roads",
                  "OPER_MAINT_LEVEL", "Project_Miles", "IN_AND_OUT", "RoadMi_OPML.xlsx")
except:
    arcpy.AddWarning("--- Unable to create summary table")
    arcpy.AddWarning("error: {}".format(sys.exc_info()[0]))
